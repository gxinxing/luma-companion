...
struct GallerySection: Identifiable, Equatable {
    ...
    /// 副标题（唯一的中文实现，屏幕直接用）。设备的 `count` 不总与实际条目数一致
    /// （实测列表报 2 但只给 1 行），如实并列而不是替设备圆谎。
    /// UInt64 直接 Int() 在被污染的超大计数上会 trap，clamping 不会。
    var subtitle: String {
        let holds: String
        switch glassesWifiFolderHolds(folder: folder) {
        case .photo: holds = "张照片"
        case .voiceRecording: holds = "条录音"
        case .videoClip: holds = "段视频"
        case .emergency: holds = "条紧急录制"
        }
        let device = Int(clamping: deviceCount)
        return items.count == device
            ? "\(items.count) \(holds)"
            : "\(items.count) \(holds)（设备报告 \(device)）"
    }
}
...
struct FileApiClient {
    ...
    enum ApiError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case let .badStatus(code):
                return "眼镜返回了 HTTP \(code)"
            case .badURL:
                return "请求地址无法解析"
            case .notText:
                return "眼镜返回的内容不是文本"
            case let .parse(reason):
                return "回复无法解析：\(FileApiClient.describe(reason))"
            case let .deleteRefused(result, info):
                return "眼镜拒绝了删除（result \(result)：\(info)）"
            case let .incompleteDownload(name, kib, got):
                return "\(name) 下载不完整 —— 应为 \(kib) KB，实收 \(got) 字节"
            }
        }
    }
    ...
    func download(_ item: GalleryItem) async throws -> URL {
        ...
        // 按文件夹落进子目录：不同文件夹同名文件（EVENT/xx.jpg 与 EMR/xx.jpg）
        // 直接平铺进 Documents 会互相覆盖，静默丢照片。
        let folderDir = Self.documentsDirectory()
            .appendingPathComponent(glassesWifiFolderName(folder: item.folder), isDirectory: true)
        try FileManager.default.createDirectory(at: folderDir, withIntermediateDirectories: true)
        let destination = folderDir.appendingPathComponent(item.basename)
